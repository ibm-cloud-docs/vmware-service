...
With single tenant and multi-tenant Cloud Director instances and its virtual data center instances, customers can migrate or deploy VMware workloads to the cloud on the IBM-hosted and managed VMware infrastructure. Customer workloads that consist of networking configurations and VMs are isolated to isolated  nd virtualized infrastructure.
...
The management domain consists of tools to support the virtual infrastructure, cloud operations, cloud automation, business continuity, and security and compliance components for {{site.data.keyword.vmware-service_short}}. {{site.data.keyword.vmware-service_short}} allocates separate workload domains per customer instance for the single tenant service and uses Cloud Director organizational isolation for multi-tenant customers within a single workload domain. Each workload domain is managed by a separate vCenter Server instance and a dedicated NSX-T Manager cluster for scalability. The vCenter Server and VMware NSX-T™ Manager components for these workload domains run in the management domain.

The management domain runs all management components in the {{site.data.keyword.vmware-service_short}} offering for both the management domain and workload domains, except for workload NSX-T Edge nodes. {{site.data.keyword.vmware-service_short}} starts with an initial management domain configuration that is extended with each workload domain deployment.
...
* Multi tenant workload domain which consistes of infrastructure and management components that are used by many customers.  Each customers VMware environment is fully isolated via Cloud Director organizations
...
Workload domains contains the following components that are installed in the workload domain:
...
There are several possible instances of customer data in motion. Customer network data flows over NSX-T virtualized overlay network segments and is isolated to the customer. NSX-T virtualized overlay networking does not provide intrinsic encryption, for data confidentiality, you must use a network encryption technique such as IPsec, SSL, or SSH. Customer disk traffic in flight is protected if customers use vSphere encryption. Customer vMotion traffic in flight is protected by vMotion encryption.
