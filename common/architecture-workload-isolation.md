...
![{{site.data.keyword.vmware-service_short}} region](../images/multi-vs-single-tenant.svg){: caption="Figure 1. {{site.data.keyword.vmware-service_short}} region" caption-side="bottom"}
...
The management domain consists of tools to support the virtual infrastructure, cloud operations, cloud automation, business continuity, and security and compliance components for {{site.data.keyword.vmware-service_short}}. {{site.data.keyword.vmware-service_short}} allocates separate workload domains per customer instance for the single-tenant service and uses Cloud Director organizational isolation for multitenant customers within a single workload domain. Each workload domain is managed by a separate vCenter Server instance and a dedicated NSX-T Manager cluster for scalability. The vCenter Server and VMware NSX-T™ Manager components for these workload domains run in the management domain.
...
{{site.data.keyword.vmware-service_short}} uses trust zones. Trust zones are a logical grouping of components that are known, controlled, and trusted. Protection boundaries are established between trust zones as described in the following table.
