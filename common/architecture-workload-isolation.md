...
* Management domain - Hosts the IBM operations tools and all the single-tenant customer management tools except for the networking edges that are used by customer workloads.
...
With single tenant Director Instances and its virtual data center instances, customers can migrate or deploy VMware workloads to the cloud on the IBM-hosted and managed VMware infrastructure. Customer workloads that consist of networking configurations and VMs are isolated to single tenant dedicated infrastructure.
...
A workload domain represents a logical unit that groups ESXi hosts managed by vCenter Server, NSX-T, and VMware Cloud Director that are dedicated to the customer workload domain instance. Workload domains exist in the boundaries of an {{site.data.keyword.cloud}} region.
...
* At least one vSphere cluster with vSphere HA with single tenant ESXi bare metal servers, networking and shared storage
...
Client metadata is stored in {{site.data.keyword.cloud}} databases (ICD) that encrypts all data both in transit and at rest.
...
Client-owned data is hosted only in the customer's workload domain instance data plane.
