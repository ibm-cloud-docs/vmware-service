...
|:---- |:----------- |:-------- |
| Management plane | {{site.data.keyword.vmware-service_short}} operational management components | The managemnt plane components are hosted on {{site.data.keyword.cloud_notm}} classic IaaS resources. Public facing IBM operational interfaces are protected by networking appliance firewalls and the use of bastion hosts. Management plane connectivity to the workload plane takes place through networking appliances. Public connectivity to all customer management interfaces are routed through Akamai DDOS and WAF security. Customers access VMware Cloud Director and associated service portal management interfaces running on the management plane by using the web UI or APIs from the public network. These interfaces are restricted and do not enable customers to transit further through the management plane network. |
| Workload plane | VMware Cloud Director Virtual Data Centers consisting of vApps, virtual machines (VM), and virtual networks | Customer workloads attach to NSX-T virtual datacenter virtualized networks that are running as overlays on the classic IaaS network. NSX-T virtualized networks are isolated to the customers account inside the customers VMware Cloud Director environments.  Customer VMs do not have access to the Workload Plane management and storage networks. Their access is restricted by using NSX-T edge appliances. Customer workload connectivity to the public internet takes place through NSX-T edge routers that are managed by the customer through a restricted VMware Cloud Director interface. Customer workload connectivity to private network takes place through IBM-managed NSX-T edge routers  are configured to limit connectivity to well-known {{site.data.keyword.cloud_notm}} private services. |
